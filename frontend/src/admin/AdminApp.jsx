...
function Sidebar({ route, setRoute }) {
  ...
  return (
    <aside
      className="w-[232px] shrink-0 h-screen sticky top-0 border-r hairline flex flex-col"
    ...
    >
      ...
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center gap-2.5">
          <div
            className="relative w-8 h-8 rounded-md grid place-items-center"
            style={{
              background: 'linear-gradient(135deg, #b6ff3c 0%, #6ee7ff 100%)',
              boxShadow: '0 0 24px rgba(182,255,60,.25)',
            }}
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#07090d" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* env selector */}
      <div className="mx-3 mb-3 chip rounded-md px-2.5 py-2 flex items-center gap-2 cursor-pointer hover:bg-white/[0.03]">
        <span className="w-1.5 h-1.5 rounded-full live-dot" style={{ background: 'var(--acc)' }} />
        <div className="flex-1 leading-tight">
          <div className="text-[11px] mono uppercase tracking-[.1em]" style={{ color: 'var(--ink-3)' }}>
            Environment
          </div>
          <div className="text-[12.5px]">Production · us-east-1</div>
        </div>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="m6 9 6 6 6-6"/></svg>
      </div>

      {/* nav */}
      <nav className="px-2 flex-1 overflow-y-auto">
        <div className="text-[10px] uppercase tracking-[.2em] px-3 mt-2 mb-1.5" style={{ color: 'var(--ink-3)' }}>
          Workspace
        </div>
        {NAV.map((n) => {
          ...
          return (
            <button
              ...
              className={
                'nav-item w-full text-left px-3 py-2 rounded-md text-[13px] flex items-center gap-2.5 mt-0.5 transition ' +
                (active
                  ? 'active bg-white/[0.04] text-white'
                  : 'text-[var(--ink-2)] hover:bg-white/[0.025] hover:text-white')
              }
            >
              ...
              {badge && (
                <span
                  ...
                  style={{
                    background: 'rgba(251,93,111,.12)',
                    color: '#fb5d6f',
                    border: '1px solid rgba(251,93,111,.3)',
                  }}
                >
                  {badge}
                </span>
        ...
        <div className="text-[10px] uppercase tracking-[.2em] px-3 mt-5 mb-1.5" style={{ color: 'var(--ink-3)' }}>
          Insights
        </div>
        {['Strategies', 'Marketplace', 'Affiliates', 'Audit log'].map((x) => (
          <button
            key={x}
            className="w-full text-left px-3 py-2 rounded-md text-[13px] text-[var(--ink-2)] hover:bg-white/[0.025] hover:text-white mt-0.5 flex items-center gap-2.5"
          >
            <span className="w-1 h-1 rounded-full" style={{ background: 'var(--ink-3)' }} />
            {x}
          </button>
        ))}
      </nav>
      ...
      <div className="p-3 border-t hairline">
        <div className="panel-2 rounded-md p-3">
          ...
          <div className="mt-1.5 flex items-center justify-between text-[12px]">
            ...
            <span className="mono text-[#34d399]">42ms</span>
          </div>
          <div className="flex items-center justify-between text-[12px]">
            <span>Queue</span>
            <span className="mono">0.3% util</span>
          </div>
          <div className="flex items-center justify-between text-[12px]">
            <span>WS clients</span>
            <span className="mono">12,041</span>
          </div>
        </div>
        <div className="mt-3 flex items-center gap-2.5 px-1">
          <div
            className="w-7 h-7 rounded-full grid place-items-center text-[10.5px] font-semibold"
            style={{
              background: 'linear-gradient(135deg, #6ee7ff44, #b6ff3c44)',
          ...
          <div className="flex-1 min-w-0">
            <div className="text-[12px] truncate">Sasha Kowal</div>
            <div className="text-[10.5px] mono truncate" style={{ color: 'var(--ink-3)' }}>
              Root admin
            </div>
          </div>
          <button className="text-[var(--ink-3)] hover:text-white p-1">
...
function TopBar({ route }) {
  ...
  return (
    <div
      className="h-12 border-b hairline px-5 flex items-center gap-4 sticky top-0 z-30"
      style={{ background: 'rgba(7,9,13,.85)', backdropFilter: 'blur(12px)' }}
    >
      ...
      <div
        className="flex items-center gap-2 px-2.5 py-1.5 rounded-md w-[280px] hidden md:flex"
      ...
      >
        ...
        <input
          placeholder="Search users, bots, invoices…   ⌘ K"
          className="bg-transparent outline-none text-[12.5px] flex-1 placeholder:text-[var(--ink-3)]"
          style={{ color: 'var(--ink)' }}
        />
      </div>
      <button className="relative chip rounded-md p-1.5 hover:bg-white/[0.04]" title="Alerts">
        ...
        <span
          ...
          style={{ background: '#fb5d6f', boxShadow: '0 0 0 2px #07090d' }}
        />
      </button>
      <div className="vsep h-5" />
      <StatusPill tone="acc" dot={true}>
...
export default function AdminApp() {
  ...
  const renderView = () => {
    switch (route) {
      case 'overview':
        return <OverviewView push={setRoute} />;
      case 'users':
        return <UsersView />;
      case 'fleet':
        return <FleetView />;
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div
          className="px-6 pb-4 pt-2 text-[11px] mono flex items-center justify-between"
          style={{ color: 'var(--ink-3)' }}
        >
          <span>© 2026 BinQuant Labs · Internal</span>
          <span>build a4b2c · region us-east-1 · 14 May 2026 · 11:42 UTC</span>
        </div>
      </main>
    </div>
  );
