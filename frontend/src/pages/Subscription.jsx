    ...
    <div className="page-enter">
      ...
      <div className="glass p-6 mb-6 relative overflow-hidden">
        ...
        <div className="relative flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            <div
              ...
              style={{
                background: 'linear-gradient(135deg, rgba(0,212,255,0.20), rgba(139,92,246,0.20))',
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[11px] uppercase tracking-wider text-dim">Current plan</span>
                <span className="badge badge-blue">Active</span>
              </div>
              <div className="text-[22px] font-semibold leading-none">
                {current.name} <span className="text-muted text-[14px] font-normal">· ${current.price}/mo</span>
              </div>
              <div className="text-[12px] text-muted mt-1.5">
                Renews on June 1, 2026 · {current.bots} bots included
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 relative">
          {[
            { label: 'Bots used', value: '5 / 10', percent: 50, color: '#00D4FF' },
            ...
            <div key={u.label}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] uppercase tracking-wider text-muted">{u.label}</span>
                <span className="num text-[12px] font-medium">{u.value}</span>
              </div>
        ...
        {PLANS.map((p) => {
          ...
          return (
            <div
              ...
              className="glass p-6 relative cursor-pointer lift"
              style={{
                ...(p.popular ? { borderColor: 'rgba(0,212,255,0.40)', boxShadow: '0 0 0 1px rgba(0,212,255,0.20), 0 20px 60px -20px rgba(0,212,255,0.30)' } : {}),
                ...(isSelected && !p.popular ? { borderColor: 'rgba(255,255,255,0.18)' } : {}),
              }}
            >
              {p.popular && (
                <div
                  className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[10.5px] font-semibold tracking-wider uppercase"
              ...
              <div className="flex items-baseline gap-1 mb-1">
                <span className="num text-[36px] font-semibold leading-none">${p.price}</span>
              ...
              <button
                className={`btn w-full justify-center ${p.popular && !isCurrent ? 'btn-primary' : ''}`}
                disabled={isCurrent}
                style={isCurrent ? { background: 'rgba(255,255,255,0.04)', color: '#8A90A2', cursor: 'default' } : {}}
              >
                {isCurrent ? 'Current Plan' : p.price > current.price ? 'Upgrade' : 'Downgrade'}
              </button>
            </div>
          );
        })}
      ...
      <div className="glass p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="text-[14px] font-semibold">Payment history</div>
            <div className="text-muted text-[12px] mt-0.5">Your past invoices</div>
          </div>
          ...
          <table className="w-full text-[12.5px]" style={{ borderCollapse: 'separate', borderSpacing: 0 }}>
            <thead>
              <tr className="text-left text-dim text-[11px] uppercase tracking-[0.08em]">
                ...
                <th className="py-2.5 pr-3 font-medium">Plan</th>
            ...
            <tbody>
              {PAYMENTS.map((p) => (
                <tr key={p.id} style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <td className="py-3 pr-3 text-muted">{p.date}</td>
                  <td className="py-3 pr-3">{p.plan}</td>
                  <td className="py-3 pr-3 num text-right">${p.amount.toFixed(2)}</td>
                  <td className="py-3 pr-3"><span className="badge badge-running">Paid</span></td>
                  <td className="py-3 pr-3 text-right">
                    <button className="btn !py-1 !px-2.5 text-[11.5px]"><Download size={12} /> Invoice</button>
                  </td>
                </tr>
              ))}
            </tbody>
