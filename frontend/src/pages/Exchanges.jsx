...
import { fmtUSD, EXCHANGES, AVAILABLE_EXCHANGES } from '../data/mockData';
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <div className="px-6 pt-6 pb-4 flex items-center justify-between" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div>
          <div className="text-[18px] font-semibold">Connect exchange</div>
          <div className="text-muted text-[12.5px] mt-1">Step {step + 1} of 4</div>
        </div>
        <button className="btn btn-ghost !p-2" onClick={onClose}><X size={16} /></button>
      </div>

      <div className="px-6 py-4">
        <Stepper steps={['Select', 'API Keys', 'Permissions', 'Done']} current={step} />
      </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {AVAILABLE_EXCHANGES.map((ex) => (
              <div
                ...
                className="glass-2 p-4 flex flex-col items-center gap-2 cursor-pointer lift"
                style={exchange?.name === ex.name ? { borderColor: 'rgba(0,212,255,0.5)', background: 'rgba(0,212,255,0.04)' } : {}}
              >
                <ExchangeLogo name={ex.name} size={42} accent={ex.accent} />
        ...
        {step === 1 && (
          <div>
            <div className="glass-2 p-3 flex items-start gap-3 mb-4" style={{ borderColor: 'rgba(0,212,255,0.30)', background: 'rgba(0,212,255,0.04)' }}>
              <ShieldCheck size={16} color="#67E5FF" />
              <div className="text-[12px] text-muted leading-snug">
                Keys are encrypted with AES-256 and never leave our secure vault. Disable withdrawal permission for added safety.
              </div>
            </div>
            <div className="label">API Key</div>
            <input className="input mono mb-3" placeholder="Paste your API key…" defaultValue="bnq_8f3a2c1e9d4b7e6f0a2b" />
            <div className="label">Secret Key</div>
            <input className="input mono" type="password" placeholder="••••••••••••••••" defaultValue="••••••••••••••••••••••••" />
          </div>
        )}
        ...
        {step === 2 && (
          <div className="flex flex-col gap-2">
            {[
              ...
              { id: 'withdraw', label: 'Withdrawals', desc: 'Not required — disable for safety', danger: true },
            ].map((p) => (
              <div key={p.id} className="glass-2 p-4 flex items-center gap-3">
                <div className="flex-1">
                  <div className="text-[13.5px] font-medium flex items-center gap-2">
                    {p.label}
                    {p.required && <span className="badge badge-blue">required</span>}
                    {p.danger && <span className="badge badge-error">unsafe</span>}
                  </div>
            ...
            <div className="glass-2 p-3 flex items-center gap-3 mt-2" style={{ borderColor: 'rgba(0,255,136,0.30)', background: 'rgba(0,255,136,0.03)' }}>
              <CheckCircle2 size={16} color="#5BFFB0" />
              <span className="text-[12.5px] text-muted">Connection test passed · Latency 84ms</span>
            </div>
          </div>
        )}
        ...
        {step === 3 && (
          <div className="text-center py-6">
            <div
              ...
              style={{ background: 'rgba(0,255,136,0.10)', border: '1px solid rgba(0,255,136,0.30)', boxShadow: '0 0 30px rgba(0,255,136,0.4)' }}
            >
            ...
            <div className="glass-2 p-4 inline-block min-w-[260px]">
              <div className="text-[10.5px] uppercase tracking-wider text-dim mb-1">Available balance</div>
              <div className="num text-[24px] font-semibold text-green">$8,420.91 <span className="text-[12px] text-muted">USDT</span></div>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="px-6 py-4 flex items-center justify-between gap-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <button className="btn" disabled={step === 0} onClick={() => setStep((s) => s - 1)} style={{ opacity: step === 0 ? 0.4 : 1 }}>
          <ChevronLeft size={13} /> Back
        </button>
        {step < 3 ? (
          <button className="btn btn-primary" disabled={step === 0 && !exchange} onClick={() => setStep((s) => s + 1)}>
  ...
  return (
    <div className="page-enter">
      <TopBar
        ...
        subtitle={`${EXCHANGES.length} connected · ${fmtUSD(totalBalance)} total balance`}
        action={
          <button className="btn btn-primary" onClick={() => setOpen(true)}>
            <Plus size={14} color="#001016" /> Add Exchange
          </button>
        }
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {EXCHANGES.map((ex) => (
          <div key={ex.id} className="glass lift p-5">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="text-dim text-[11px]">synced {ex.lastSync}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="glass-2 p-4 mb-4">
              ...
              <div className="num text-[24px] font-semibold">{fmtUSD(ex.balance)} <span className="text-[12px] text-muted">USDT</span></div>
            </div>
